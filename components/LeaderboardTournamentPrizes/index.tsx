...
export default function LeaderboardTournamentPrizes({ className, leaderboard, ...props }: Props) {
  ...
  return leaderboard.rewards.length === 0 ? null : (
    <section {...props} className={classNames(styles.leaderboardTournamentPrizes, className)}>
      <div className={styles.header}>
        <h4><FormattedMessage defaultMessage="Prizes Table" /></h4>
      </div>

      <div className={styles.body}>
        {leaderboard.rewards.sort((a, b) => b - a).map((reward, index) => (
